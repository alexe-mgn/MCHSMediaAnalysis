...
class MCHSPageParser:
    ...
    def parse(self) -> List[Dict[str, Any]]:
        """
        Fill .news with results of parse_item on each element of base_xpath(tree)
        """
        news = []
    ...
    def parse_item(self, element: lxml.etree.Element) -> Dict[str, Any]:
        """
        Parse news HTML element and return data dict.
        """
        item = {}
        for k, v in [("id", self.parse_id),
                     ("title", self.parse_title),
                     ("date", self.parse_date),
                     ("category", self.parse_category),
                     ("category_id", self.parse_category_id),
                     ("image", self.parse_image)]:
            item[k] = v(element)
        return item

    def parse_id(self, element: lxml.etree.Element) -> Optional[int]:
        href = self.item_id_xpath(element)[0]
        if href is not None:
            return int(href.strip('/').split('/')[-1])
        return None

    def parse_title(self, element: lxml.etree.Element) -> Optional[str]:
        return self.item_title_xpath(element)[0]

    def parse_date(self, element: lxml.etree.Element) -> Optional[datetime.datetime]:
    ...
    def parse_category(self, element: lxml.etree.Element) -> Optional[str]:
        return self.item_category_xpath(element)[0]

    def parse_category_id(self, element: lxml.etree.Element) -> Optional[str]:
        return self.item_category_id_xpath(element)[0].split('=')[-1]

    def parse_image(self, element: lxml.etree.Element) -> Optional[str]:
...
class MCHSNewsParser:
    ...
    def parse_text(self, element: lxml.etree.Element) -> Optional[str]:
        return '\n'.join(self.text_xpath(element))

    def parse_categories(self, element: lxml.etree.Element) -> Optional[List[str]]:
        return list(self.categories_xpath(element))

    def parse_category_ids(self, element: lxml.etree.Element) -> Optional[List[str]]:
        return [e.strip('/').split('=')[-1] for e in self.category_ids_xpath(element)]

    def parse_tags(self, element: lxml.etree.Element) -> Optional[List[str]]:
        return list(self.tags_xpath(element))

    def parse_tag_ids(self, element: lxml.etree.Element) -> Optional[List[int]]:
